...
export default function OSShell() {
  ...
  // only show icons for installed app IDs
    ...
    <>
      ...
      <div style={{ width: '100vw', height: '100vh', background: C.bg,
        ...
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
          ...
          {/* Desktop icons — only unlocked apps */}
        ...
        {/* ── Taskbar ── */}
        <div style={{ height: 44, background: C.surface,
          ...
          <div style={{ flex: 1, display: 'flex', gap: 4 }}>
            {wins.map(w => (
              <button key={w.id} onClick={() => focusWin(w.id)}
                style={{ padding: '3px 12px', fontSize: 11, borderRadius: 4,
...
function DesktopIcon({
  icon, label, accent = '#00e5ff', badge = 0, onClick
...
function OsWindow({ win, onClose, onFocus, onMove }: {
  ...
  const startDrag = (e: React.MouseEvent) => {
    ...
    const move = (ev: MouseEvent) => {
      ...
      onMove(ev.clientX - dragRef.current.ox,
